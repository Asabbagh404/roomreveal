...
export function MaskSurface() {
  ...
  const hasRun = state.maskDraft !== undefined;
  const busyOrErrored =
    state.waitPhase !== undefined || state.error !== undefined;

  useEffect(() => {
    if (state.step !== "mask" || hasRun || busyOrErrored) return;
    ...
    // Re-run only when the attempt (epoch) changes or detection is reset.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state.step, state.epoch, hasRun, busyOrErrored]);
