...
- 2026-07-12 : **Vérif live (FAL_KEY, Playwright, backend détection local GPU)**. Pipeline mécanique 100 % OK : upload cuisine → détection (masque 39,5 %, **0 % de gris** = binaire AD-7) → validation → « Étape 3 sur 4 : Pièce vide » + WaitPanel « Génération de la Pièce vide… » → inpaint via proxy (POST 200 → GET 202 → GET 200) → Pièce vide 1024×768 (dims canoniques), 0 erreur console. Preuve serveur (sharp sur input/masque/output) : diff **48,9** en zone blanche vs **1,5** en zone noire → **polarité correcte + hors-masque intact**. MAIS `flux-pro/v1/fill` **recomplète des meubles** (SM-2) — prouvé indépendant du prompt (bench 4 modèles sur le même couple image+masque). Voir [[inpaint-declutter-quality]].
- 2026-07-12 : **Changement de modèle inpaint → `fal-ai/bria/eraser`** (object-eraser) suite au bench live (flux fill ❌ recomplète, LaMa ⚠️ bave, Bria ✅ supprime proprement). Adaptateur adapté (input `mask_url`+`mask_type:"manual"`, sortie `image.url`, plus de prompt) ; `EMPTY_ROOM_PROMPT` conservé mais inutilisé. Re-vérif live à travers l'app : îlot/colonnes/meubles bas supprimés → sol+mur nus, 1024×768, 0 erreur. Résidus (TV, objets plan) = couverture détection, sujet séparé. 123 tests, tsc/lint/build verts.
