...
- 2026-07-13 : glisser-boîte + undo commités en local (`85639ce`, non pushé), avec le travail concurrent editModify/sweep entremêlé (fichiers partagés).
- 2026-07-14 (retour user) : **polish loader box-drag** — le point qui pulse est au **centre du carré** tracé (plus au point de relâché) ; le **carré reste visible jusqu'à l'arrivée du résultat** (caché dans l'effet de résolution quand `selecting` retombe → le pulse silhouette prend le relais ; aussi caché par `changeTool`). Purement visuel dans MaskCanvas (endStroke : anchor centre + garde l'opacité pour box ; clic reste au point). 12 tests MaskCanvas + tsc verts, lint 5.6/5.7 propre.
