...
- 2026-07-12 : **Calibration du mouvement (retour utilisateur : « rêve fiévreux », meubles qui se matérialisent sur place)**. Bench live de 3 prompts de mouvement (2 tirages chacun) sur le même couple Pièce vide/Photo → l'utilisateur retient la variante **A (descend du plafond + glisse des côtés)**. Câblé : `REVEAL_MOTION_PROMPT` = variante A + nouveau `REVEAL_NEGATIVE_PROMPT` anti-morphing (dissolve/materialize/fade), envoyé en `negative_prompt` dans `video.ts` (`@ts-expect-error` : champ API kling valide mais absent du type généré `@fal-ai/client`). Variantes B/C conservées en alternatives documentées dans `prompts.ts`. Échantillons dans `resultat/` (gitignored). 139 tests, tsc/lint/build verts.
