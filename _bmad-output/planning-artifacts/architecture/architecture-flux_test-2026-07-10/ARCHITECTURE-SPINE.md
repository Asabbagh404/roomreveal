...
- **Rule:** ordre du pipeline fixe (Détection → Inpainting → Génération FLF) ; première frame = Pièce vide, dernière frame = Photo originale strictement intouchée. Tout modèle vidéo de remplacement doit offrir un mode FLF dédié (start **et** end frame contraints) — jamais d'image-to-video simple ni de « tail image » indicatif.
  - **[AMENDEMENT 2026-07-24 — Story 4.8, backend `motion-brush` uniquement]** Le backend vidéo alternatif `VIDEO_BACKEND=motion-brush` (Kling Motion Brush reverse-motion) **relâche la contrainte de première frame** : sa première frame est **générée par le modèle** (pièce quasi-vide après la sortie des meubles), **pas** notre inpaint (`generation.emptyRoom` n'est pas consommée par ce backend). La dernière frame est le **rendu du modèle ré-encodé** — visuellement la photo meublée, mais **PAS pixel-exact** : le clip reversé est un ré-encodage avec perte du rendu Kling, pas la photo canonique bit-pour-bit. L'invariant strict de dernière frame d'AD-1 est donc **APPROXIMÉ par ce backend, pas tenu au pixel près**. Mécanique : on génère le clip meublé→vide (départ = photo parfaitement contrainte, meubles sortant via `dynamic_masks` + trajectoires) puis on **inverse temporellement le MP4**, donc la fin retombe visuellement sur la photo (au ré-encodage près). Cause structurante : sur Kling pro, `tail_image_url` est **mutuellement exclusif** avec `dynamic_masks`/`static_mask_url` — on ne peut pas contraindre la dernière frame *et* piloter le mouvement par masque ; le reverse-motion est le contournement. Note modèle : les champs Motion Brush (`dynamic_masks`+`static_mask_url`) sont portés par **v1.5 pro** (`fal-ai/kling-video/v1.5/pro/image-to-video`) ; v1.6 pro les a retirés (son type d'entrée n'a que `tail_image_url`). Relâchement/approximation **assumés et pré-acceptés** en choisissant l'approche Motion Brush, **valables pour ce seul backend**. Le défaut `flf` (veo 3.1 lite) conserve la contrainte FLF stricte (première = Pièce vide, dernière = photo), intouché. Voir aussi AD-9 : masques par instance + MP4 inversé repassent par `uploadArtifact` (le reveal reste une URL fal) ; la route locale `/reverse` est un traitement transitoire, pas un stockage.
...
- **Rule:** `src/pipeline/` est le **seul importeur** de `@fal-ai/client`, upload storage compris : il expose `uploadArtifact(blob) → URL fal` en plus des trois adaptateurs (`detect`, `inpaint`, `video`), tous derrière la même interface passive (AD-12) avec erreurs normalisées (AD-8). Les couches amont manipulent des `Blob` locaux jusqu'à la frontière pipeline ; toute URL fal naît dans `pipeline/`. Les trois adaptateurs ne consomment que des URLs fal — seul `uploadArtifact` accepte un `Blob` (la couche effectrice uploade une fois, mémoïse dans `generation`) : `detect(photoUrl)`, `inpaint(photoUrl, maskUrl)`, `video(emptyRoomUrl, photoUrl)`. Contrat de sortie de `detect` : `{ initialMask: <PNG binaire unique, dimensions et sémantique AD-2/AD-7> | null, categories: string[] }` — la composition des N segments SAM (union, seuil de confiance, mise aux dimensions canoniques) appartient à `detect`, jamais à l'UI ni au reducer ; `initialMask === null` est LA définition canonique du cas FR-16 ; `categories` reste interne, jamais exposée en UI (décision UX canonique). Les IDs de modèles sont référencés par rôle dans un module de config unique — changer de modèle = éditer ce fichier, rien d'autre.
  - **[AMENDEMENT 2026-07-11 — backend de détection enfichable, Story 2.2]** La détection accepte désormais **deux backends**, sélectionnés par `DETECT_BACKEND` (env `NEXT_PUBLIC_DETECT_BACKEND` = `fal` | `local`, défaut `fal`) — le swap localisé que cette AD anticipait déjà (cf. « Fallback Replicate »). Le **contrat de sortie `detect` et l'interface passive sont inchangés** : `local` fournit `detectLocal(blob, opts)` (dans `pipeline/`) qui POST la copie de détection + toute la liste `FURNITURE_CATEGORIES` à un service Grounded-SAM auto-hébergé (`local-detect/` : Grounding DINO — qui, lui, accepte **tous** les concepts en un seul prompt — → boîtes → SAM → **union** des masques), et renvoie le masque unionné en data URL (`initialMask`), `null` sur HTTP 204 (FR-16). Motivation : fal SAM 3 ne prend qu'un concept par appel, donc couvrir une pièce entière coûte cher ; le backend local unionne toutes les catégories **gratuitement**. Le module `@fal-ai/client` reste importé **uniquement** par le chemin fal (AD-5 tenu) ; `detectLocal` n'utilise pas fal. **AD-4 non affecté** : `FAL_KEY` reste serveur-only ; l'URL locale (`NEXT_PUBLIC_LOCAL_DETECT_URL`, localhost dev) n'est pas un secret. Le chemin fal reste le défaut ; le local est expérimental (dev) et vérifié séparément (GPU requis).
